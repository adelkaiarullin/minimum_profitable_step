...
$$
(R_{a} - 1) \cdot P_{a} \cdot c_{a} + \beta \cdot \frac{P_{a} \cdot c_{a}}{P_{b} \cdot c_{b}} \cdot (R_{b} - 1) \cdot P_{b} \cdot c_{b} > 2 \cdot \text{комиссия} \cdot (1 + N_{hedge}) \quad (2)
$$
...
$$
\ln R_{a} - \beta \cdot \ln R_{b} = \Delta \text{spread} = \frac{2 \cdot k \cdot \text{commission} \cdot (1 + N_{hedging})}{P_{a} \cdot c_{a}} \quad (4)

Equation (4) is written as the final equation for finding the minimum profitable step $\Delta \text{spread} = step, the term $\frac{2 \cdot k \cdot \text{commission} \cdot (1 + N_{hedging})}{P_{a} \cdot c_{a}}$ is multiplied by k to account for slippage and other execution risks.
